...
def analyze_threat(user_question, log_context, api_key):
    ...
    try:
        ...
        if "429" in error_msg or "ResourceExhausted" in error_msg:
            return jsonify(
                {
                    "response": "🛑 NOPE! AI: We've reached the free-tier rate limit (15 requests/min). Please wait 60 seconds or use a different API key."
                }
            )
        return jsonify(
            {"response": f"AI Analyser temporarily unavailable: {error_msg}"}
        )
